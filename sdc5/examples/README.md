...
This directory contains working examples of SDC5-compliant data models and instance documents.

**All examples validate against `sdc5.xsd`** - These are tested, working implementations.

## Directory Structure

### Basic Examples (`basic/`)

Simple, introductory examples demonstrating core SDC5 concepts:
...
*Perfect for learning SDC5 basics.*
...
Complex examples showing advanced SDC5 features:
...
*For experienced SDC5 implementers.*

## Validation

All examples should validate against the SDC5 schema:

```bash
# Validate a single file
xmllint --schema ../schemas/sdc5.xsd --noout basic/patient-record.xml

# Validate all examples
for f in **/*.xml; do
    echo "Validating $f..."
    xmllint --schema ../schemas/sdc5.xsd --noout "$f"
done
```

## Mandatory instance_id

As of the Beale-Sovereignty update, every SDC5 data instance **must** include an `instance_id` element. The SDC ecosystem uses CUID2 identifiers. New examples must include this element.

## Example Structure

Each example typically includes:

1. **XML Schema** - Defines the data model (using XSD restriction of sdc5.xsd)
2. **XML Instance** - Sample data conforming to the schema
3. **Comments** - Inline documentation explaining choices

Some examples may be complete XML instances that validate directly against sdc5.xsd without a custom schema.
...
```bash
xmllint --schema ../schemas/sdc5.xsd --noout your-example.xml
```
